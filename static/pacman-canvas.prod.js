...
/* ----- Global Variables ---------------------------------------- */
	...
	function getHighscore() {
		setTimeout(highscore_get,30);
	}

	function click_submit(playerName) {
			var csrfToken = $("input[name=_csrf]").val();

			$.ajax({
			url: '/checkname',
			data: {_csrf: csrfToken, playerName: playerName},
			async: true,
        ...
        $('#form-validater').html("");
			...
			success: function (data) {
				if (data.valid) {
	        $('#score-submit').prop('disabled', false);
	        $('#form-validater').html("");
	        $('#highscore-form').html('<span class="button" id="show-highscore">View Highscore List</span>');
	        highscore_add(playerName.toLowerCase(), game.score.score, game.level);
				} else {
	        $('#score-submit').prop('disabled', false);
	        $('#form-validater').html("Please enter a valid Steem username<br/>");
				}
			}
		});
	}

	function highscore_get() {
		$.ajax({
			url: '/highscores',
			async: true,
			 beforeSend: function(xhr){
			...
			dataType: "json",
			success: function (data) {
		    data.forEach(function(sortedItem){
		      $("#highscore-list").append("<li>"+sortedItem.username+"<span id='score'>"+sortedItem.highscore+"</span></li>");
		    });
			}
		});
	}

	function highscore_add(n, s, l) {
		var csrfToken = $("input[name=_csrf]").val();

		$.ajax({
			url: '/savescore',
			data: {n: n, s: s, l: l, _csrf: csrfToken},
			async: true,
			 beforeSend: function(xhr){
				if (xhr.overrideMimeType) xhr.overrideMimeType("application/json"); 
			},
	...
	function addHighscore() {
      ...
      highscore_add('test2', game.score.score, game.level);
	}
...
function checkAppCache() {
	...
	$(document).ready(function() {	
		...
		$('body').on('click', '#score-submit', function(e){
			var playerName = $('#playerName').val();
			$('#score-submit').prop('disabled', true);
			if (playerName === "" || playerName === undefined) {
				$('#form-validater').html("Please enter a valid Steem username<br/>");
			} else {
				click_submit(playerName);
			}
		});
